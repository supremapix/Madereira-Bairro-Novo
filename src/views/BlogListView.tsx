        ...
        <div className="max-w-7xl mx-auto space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filtered.map((post) => (
              <div
                key={post.id}
                className="bg-stone-900 rounded-3xl border border-stone-800 overflow-hidden flex flex-col justify-between hover:border-amber-500/40 transition-colors"
              >
                <img
                  src={post.imageUrl}
                  alt={post.imageAlt}
                ...
                <div className="p-6 space-y-3 flex-1 flex flex-col justify-between">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-[11px] text-stone-400">
                      <span className="font-extrabold uppercase text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-md">
                        {post.category}
                      </span>
                      <span>{post.readTime} de leitura</span>
                    </div>

                    <h2 className="text-lg font-bold text-stone-100 line-clamp-2">
                      {post.title}
                    </h2>
